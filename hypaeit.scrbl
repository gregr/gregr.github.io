...
A program must be comprehensible for its correctness to be apparent and for it to be maintainable.  It must also execute efficiently in order to operate within a resource budget.  Programs typically cannot be written in a manner that maximizes both comprehensibility and efficiency.  Maximizing comprehensibility requires limiting a program's complexity to its essential concerns, but this typically requires the use of abstractions that hide the inessential details of how computations are carried out.  Since a program directly controls only the factors it describes, such a maximally comprehensible program cannot precisely allocate computational resources for the purpose of maximizing efficiency.

Optimizing compilers can automatically mitigate some of this loss of efficiency, allowing programs to continue using abstractions without paying as much in performance.  However, such automation is not one-size-fits-all, with competing factors such as resource consumption, effectiveness and predictability.  Due to the large search space, as problem complexity grows, identifying optimization opportunities becomes disproportionately expensive and unreliable.  Small changes to a program may lead to significant unpredictable changes in performance due to this fragility.

Having access to runtime data, just-in-time compilers cope with a smaller search space than ahead-of-time compilers, allowing a better effectiveness to analysis ratio.  But happening at runtime, they are also more resource constrained, often only having time to perform simple optimizations.  Being dependent on predictable runtime data flow and hotspots, they are also less predictable regarding if, when, and how optimizations manifest.

Problem-specific knowledge can also help tame the optimization search space.  Though specialized optimizers exist for some domains, in general the responsibility for applying problem-specific knowledge falls to the programmer @(cite death-opt-compilers).  Armed with the right priorities and appropriate profiling information, the programmer can focus on just the problematic portions of a program, manually tuning them to dramatically improve performance.  And because these improvements are explicitly encoded, they will not unpredictably vanish.  This tuning may be algorithmic or involve specialization: eliminating expensive abstractions using statically available information.  For programs written in high level languages, specialization may even involve rewriting critical portions in a lower level language.  Complex algorithms and specialization greatly reduce comprehensibility, compromising correctness and maintainability.


### Proposal

In terms of the comprehensibility and efficiency tradeoff, general-purpose automation and problem-specific manual tuning each seem to be strong where the other is weak.  Is there some way to reconcile the two approaches to get the best of both?  It may be possible to achieve this by designing tools for maintaining **multiple semantically-equivalent views** of the same program.  Some views would consolidate meaning as a high level, maximally comprehensible program.  Other views would robustly elaborate the program's computational details for efficiency.  We would like to experiment with this idea.

For the purpose of experimentation, we propose a minimal set of tools that includes:
